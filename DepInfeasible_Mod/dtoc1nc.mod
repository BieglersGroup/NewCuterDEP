...
subject to cons1dep{t in 1..1}:
	(sum {k in 0..ny*nx-1} c[(k div nx)+1,k-nx*(k div nx)+1]*y[t,(k div nx)+1]*x[t,k-nx*(k div nx)+1]+0.5*y[t,1] + 0.25*y[t,2] - y[t+1,1] + sum {i in 1..nx} b[1,i]*x[t,i])
	-(sum {k in 0..ny*nx-1} c[(k div nx)+1,k-nx*(k div nx)+1]*y[t,(k div nx)+1]*x[t,k-nx*(k div nx)+1]+0.5*y[t,1] + 0.25*y[t,2] - y[t+1,1] + sum {i in 1..nx} b[1,i]*x[t,i])^2 = 100;
